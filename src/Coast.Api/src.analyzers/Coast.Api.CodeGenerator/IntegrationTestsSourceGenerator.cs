...
namespace Coast.Api.CodeGenerator;
...
public class IntegrationTestsSourceGenerator : ISourceGenerator
{
        ...
        try
        {
...

            {
                ...
                foreach (var typeSymbol in typeSymbols)
                {
                    ...
                    {
                        "Autofac",
                        "Coast.Api.Primary.Contracts.Bases",
                        "Xunit"
                    };
                    var contextType = "";
                    var sendString = "";
    ...
    private static List<ITypeSymbol> GetTypeSymbols(ISymbol symbol)
    {
        ...
        else if (symbol is ITypeSymbol typeSymbol)
        {
            if (typeSymbol.AllInterfaces.Any(e =>
                    e.ToDisplayString().Contains("Coast.Api.Primary.Contracts.Bases.ITestable")))
            {
        ...
        string sendString)
    {
        return
            $@"
    {usingString}
namespace Coast.Api.IntegrationTests.AutoGenerated;
