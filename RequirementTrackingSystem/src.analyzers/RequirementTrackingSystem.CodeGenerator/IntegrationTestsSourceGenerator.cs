...
namespace RequirementTrackingSystem.CodeGenerator;
...
public class IntegrationTestsSourceGenerator : ISourceGenerator
{
        ...
        try
        {
...

            {
                ...
                foreach (var typeSymbol in typeSymbols)
                {
                    ...
                    {
                        "Autofac",
                        "RequirementTrackingSystem.Primary.Contracts.Bases",
                        "Xunit"
                    };
                    var contextType = "";
                    var sendString = "";
    ...
    private static List<ITypeSymbol> GetTypeSymbols(ISymbol symbol)
    {
        ...
        else if (symbol is ITypeSymbol typeSymbol)
        {
            if (typeSymbol.AllInterfaces.Any(e =>
                    e.ToDisplayString().Contains("RequirementTrackingSystem.Primary.Contracts.Bases.ITestable")))
            {
        ...
        string sendString)
    {
        return
            $@"
    {usingString}
namespace RequirementTrackingSystem.IntegrationTests.AutoGenerated;
